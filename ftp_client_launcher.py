...
def connect():
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pyautogui.alert('Wi-Fi not available on this system')
                return
    ...
    try:
        ...
        pyautogui.alert('Wi-Fi not connected on this system')
        print('Wi-Fi is not connected. Connect to your phone')
        return
    
    # If all conditions are positive the final Url is set
    URL = 'ftp://'+gateway.split(':')[-1].strip()+':2221'
    subprocess.Popen('explorer '+URL)
    pyautogui.alert('Opening client: '+URL, 'Opening client')
